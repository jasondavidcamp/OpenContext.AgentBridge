...
$scratchRoot = Join-Path $repoRoot ".agentbridge\cline-canary-workspaces"
...
$logRoot = Join-Path $repoRoot ".agentbridge\cline-canary-logs"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        if ($SkipImageBuild) {
            throw "Docker image not found: $Image. Build docker\cline-dotnet.Dockerfile or remove -SkipImageBuild."
        }

        Write-Section "Build Cline Docker Image"
        docker build -f .\docker\cline-dotnet.Dockerfile -t $Image .
        if ($LASTEXITCODE -ne 0) {
    ...
    Write-Section "Cline Docker Edit Canary"
    & (Join-Path $PSScriptRoot "Start-ClineDocker.ps1") `
        ...
        -TimeoutSeconds 420 `
        -Thinking low `
        -Message "Inspect examples/sandbox-project/README.md and $programPath. Modify Greeter.CreateGreeting in $programPath so the returned greeting includes the phrase 'from AgentBridge' while preserving the supplied name. Do not edit any other source files. Run: dotnet run --project examples/sandbox-project/SandboxApp -- AgentBridge"
    ...
    if ($validationText -ne "Hello, AgentBridge from AgentBridge!") {
        throw "Cline Docker canary produced unexpected validation output."
    }
    ...
    if (-not $diffText.Contains("from AgentBridge")) {
        throw "Cline Docker canary diff did not include the expected greeting change."
    }
    ...
    [pscustomobject]@{
        ...
        cline_url_from_container = $containerBase
